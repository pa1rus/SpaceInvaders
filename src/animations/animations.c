...
void UpdateAnimations()
{
    ...
    if (!gameOver)
    {
        ...
        if (playerAnimation.frameCounter >= playerAnimation.frameSpeed)
        {
            ...
            if (playerAnimation.currentFrame >= 4)
                playerAnimation.currentFrame = 0;
...
void UpdateBackground()
{
    ...
    if (!pause)
    {
        ...
        if (scrolling0 >= background0.height)
            scrolling0 = 0;
        if (scrolling1 >= background1.height)
            scrolling1 = 0;
    }
    ...
    if (gameOver)
    {
        ...
        if (scrolling0Speed > 8)
            scrolling0Speed -= 10 * GetFrameTime();

        if (scrolling1Speed > 16)
            scrolling1Speed -= 20 * GetFrameTime();
    }
}
